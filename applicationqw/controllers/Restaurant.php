...
class restaurant extends CI_Controller {
    ...
    public function index() {
     ...
     if ($this->ion_auth->logged_in()) {   
        ...
        $coutry=$this->hb_countries_model->getCountryByName($country_name);        
        ...
        if (!empty($lat) && !empty($lng)) {
            $this->load->model('restaurant_branches_model');
            $restaurants = $this->restaurant_branches_model->get_restaurant_near_branches($lat, $lng,$coutry);
...
//            exit();
    ...
    public function restaurant_detail() {
        ...
        $data['restaurants'] = $this->restaurant_model->get_restaurants($country_id);
        $data['restaurant_detail'] = $this->restaurant_model->get_by_id($restaurant_id);
        if(empty($data['restaurant_detail']))
         {
             redirect('main');
         }
        ...
        $data['all_services']= $this->restaurant_model->get_all_services();
        
         $data['restaurant_rating'] = $this->restaurant_model->get_restaurant_rating_by_id($restaurant_id);
        ...
        $data['promotion']=  $this->restaurant_promotional_banner_model->count_promotional_id($restaurant_id);
        $data['retaurant_menus']=  $this->restaurant_model->get_restaurant_menus($restaurant_id);
...
//        $restaurant_id = $this->uri->segment(4);
    ...
    public function check_user_exist()
	{
		...
		if(!empty($username)) {
		  if($result>0) {
		       echo "<p style='color:#a94442; margin-top:5px; margin-bottom:0; font-size:85%; font-weight: 500; font-family:'Arial', sans-serif;'>Username Already Exist.</p>";
		  }else{
	...
	public function check_name_exist()
	{
		...
		if(!empty($name)) {
		  if($result>0) {
		       echo "<p style='color:#a94442; margin-top:5px; margin-bottom:0; font-size:85%; font-weight: 500; font-family:'Arial', sans-serif;'> Restautrant Name is Exist.</p>";
		  }else{
		      echo "<p style='color:green; margin-top:5px; margin-bottom:0; font-size:85%; font-weight: 500; font-family:'Arial', sans-serif;'> Name Available.</p>";
		  }
		}
		...
		$post= $this->input->post();
                ...
                $result_name= $this->restaurant_model->restaurant_name_exist($restaurant_name);
                $result_username= $this->restaurant_model->user_exist($user_name);
		
                
                
	            if ($result_name >0) {
	            $this->session->set_flashdata('error', "Restaurant Name already exists");
