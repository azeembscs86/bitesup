...
//    }
    ...
    public function count_user_followers($user_id)
    {
        $sql = "SELECT count(user_friend_list.id) as total_followers FROM user_friend_list WHERE user_id=$user_id and status=1";       
       $result = $this->db->query($sql)->row();
       return $result; 
    }
    
    
    public function count_user_followings($user_id)
    {
        $sql = "SELECT count(user_friend_list.id) as total_followings FROM user_friend_list WHERE user_id=$user_id and status=2";       
       $result = $this->db->query($sql)->row();
       return $result; 
    }
}
